...
def display_df(df, colors='viridis'):
    # only use half of the rules (symmetry)
    dfT = df[:8].T
    tab = sns.heatmap(dfT, xticklabels=dfT.columns, annot=True, cmap=colors)
...
plt.figure()
mat = display_df(info_rules[unq], 'Greens')
plt.title('Unique')
plt.savefig(snakemake.output.unq)

plt.figure()
mat = display_df(info_rules[red], 'Reds')
plt.title('Redundancy')
plt.savefig(snakemake.output.red)

plt.figure()
mat = display_df(info_rules[syn], 'Blues')
plt.title('Synergy')
