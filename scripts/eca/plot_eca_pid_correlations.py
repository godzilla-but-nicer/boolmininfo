...
df_list = [bld, wedge, ccs, imin, pm]
labels = [r'$BLD$', r'$I_{\wedge}$', r'$I_{ccs}$', r'$I_{min}$', r'$I_{\pm}$']
...
# adding noise to get correlations to work
noise = np.random.uniform(low=-1e-6, high=1e-6, size=(256, 5))
for term in term_dfs:
    term_dfs[term] += noise
...
sns.heatmap(overall_correlations, annot=True, xticklabels=new_labels,
            yticklabels=new_labels, cbar_kws={'label': 'Correlation'})
...
plt.xlabel('PID pair')
plt.ylabel('Correlation across atoms')
